...
class BaweNeuralExtractor(BaseFeatureExtractor):
    def __init__(self):
        bawe_train_stats = pickle.load(
            resource_stream("notebooks", "bawe_train_stats.p")
        )
        self._pos_vocab = bawe_train_stats["pos_vocab"]
        self._embedding = Embedding(
            len(self._pos_vocab), 10, padding_idx=self._pos_vocab["<pad>"]
        )
        self._embedding.load_state_dict(
            torch.load(resource_stream("notebooks", "bawe_embedding_sd.pt"))
        )
        ...
        self._style_encoder.load_state_dict(
            torch.load(
                resource_stream("notebooks.resources", "bawe_style_encoder_sd.pt")
            )
        )
    ...
    def extract(self, text: str) -> ndarray:
        ...
        with torch.no_grad():
            ...
            packed_tensor = pack_padded_sequence(
                embed_tensor, sent_lens, enforce_sorted=False
            )
