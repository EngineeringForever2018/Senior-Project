...
class EqualOpDataLoader:
    ...
    def _sample_point(self, label):
        ...
        return (
            data[selected_slice],
            sentence_lengths[selected_slice],
            chosen_label_cloned[selected_slice],
        )
...
class Preprocessor:
    ...
    def _pack(first, second):
        ...
        def reform(data, sentence_lengths):
            ...
            return torch.transpose(reshaped, 0, 1), torch.reshape(
                sentence_lengths, [-1]
            )

        first_data, first_sentence_lengths = reform(first_data, first_sentence_lengths)
        second_data, second_sentence_lengths = reform(
            second_data, second_sentence_lengths
        )

        first_data = pack_padded_sequence(
            first_data, first_sentence_lengths, enforce_sorted=False
        )
        second_data = pack_padded_sequence(
            second_data, second_sentence_lengths, enforce_sorted=False
        )
    ...
    def _move(self, first, second):
        ...
        return (first_data.to(self.dev), first_labels.to(self.dev)), (
            second_data.to(self.dev),
            second_labels.to(self.dev),
        )
...
if __name__ == "__main__":
    ...
    train_sentence_lengths = torch.load(
        join(preprocessed_data_dir, "bawe_train_sentence_lengths.pt")
    )
    ...
    valid_sentence_lengths = torch.load(
        join(preprocessed_data_dir, "bawe_valid_sentence_lengths.pt")
    )
    ...
    valid_dl = EqualOpDataLoader(
        valid_set, bs=64, pipeline=Preprocessor(torch.device(0))
    )
