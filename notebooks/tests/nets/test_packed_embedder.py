...
class TestPackedEmbedder:
    ...
    @pytest.mark.parametrize('x, lengths, expected_last_items',
    ...
    def test_packed_embedder_retains_position(self, x, lengths, expected_last_items):
        ...
        assert torch.all(torch.eq(get_last_items(unpacked_embedding, lengths), expected_last_items))
